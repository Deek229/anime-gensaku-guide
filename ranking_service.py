...
def _load_cache(path: Path, *, ignore_ttl: bool = False) -> dict[str, Any] | None:
    if not path.exists():
        return None
    try:
        payload = json.loads(path.read_text(encoding='utf-8'))
        if not ignore_ttl and time.time() - payload.get('fetched_at', 0) > RANK_CACHE_TTL_SEC:
            return None
        return payload
...
def get_ranking(period: str = 'd', target: date | None = None) -> dict[str, Any]:
    ...
    try:
        rank_rows = fetch_ranking(target, period)[:RANKING_DISPLAY_LIMIT]
        ncodes = [r['ncode'] for r in rank_rows]
        novels = fetch_novels(ncodes)
        items = [_enrich_row(r, novels.get(str(r['ncode']).upper())) for r in rank_rows]
        ...
        stale = _load_cache(cache_path, ignore_ttl=True)
        if stale:
            stale = dict(stale)
            stale['stale'] = True
